...
namespace RV
{
     ...
     void calcExtrinsicCameraParam(vector<double> &_camPos, double &tilt, vector<Point3f> &objectPoints, vector<Point2f> &imagePoints, Mat &_camMat, Mat &_distCoef);
     const ObjectPos calcObjectDistance(double tilt, double h, const cv::Point2d &focalLength, const cv::Point2d &principalPt, const cv::Point2d &pixelPt);
     void fillInContours(vector<vector<Point> > &contours, vector<Vec4i> &hierarchy, int contourIdx, Mat& dst);
}
