...
void RV::calcExtrinsicCameraParam(vector<double> &_camPos, double &tilt, vector<Point3f> &objectPoints, vector<Point2f> &imagePoints, Mat &_camMat, Mat &_distCoef)
{
    Mat distCoeffs(4, 1, CV_64FC1, (double *)_distCoef.data);
    Mat rvec, tvec;

    if(objectPoints.size() == imagePoints.size())
    {
        solvePnP(objectPoints, imagePoints, _camMat, distCoeffs, rvec, tvec);

        Mat R;
        Rodrigues(rvec, R);
        Mat Rinv = R.inv();

        Mat camPos = -Rinv * tvec;
        double* p = (double *)camPos.data;
        for(int i = 0; i < 3; i++)
            _camPos.push_back(p[i]);
...
const ObjectPos RV::calcObjectDistance(double tilt, double h, const cv::Point2d &focalLength, const cv::Point2d &principalPt, const cv::Point2d &pixelPt)
{
...
//    cout << "principal: " << principalPt.x << " " << principalPt.y << endl;
...
void RV::fillInContours(vector<vector<Point> > &contours, vector<Vec4i> &hierarchy, int contourIdx, Mat& dst)
{
    if(dst.channels() != 1)
    {
        cout << "In function fillInContours: dst image must be 8-bit, 1 1channel." << endl;
        exit(1);
    }

    Mat drawing = Mat::zeros(dst.size(), CV_8UC1);
    cv::drawContours(dst, contours, contourIdx, Scalar(74), 2, LINE_8, hierarchy, 0, Point());

    cv::Point COM(0, 0);
    size_t contourSize = contours[contourIdx].size();
    for(size_t s = 0; s < contourSize; s++)
    {
        COM.x += contours[contourIdx][s].x;
        COM.y += contours[contourIdx][s].y;
    }
    ...
    const int cX = COM.x, cY = COM.y;

    dst.data[cY * dst.cols + cX] = 255;

    for( int y = cY, idxY = cY * drawing.cols ; y > 0 ; y--, idxY -= drawing.cols )
    {
        for( int x = cX ; x < dst.cols ; x++ )
        {
            if(dst.data[idxY + x] == 74) break;
            dst.data[idxY + x] = 255;
        }
        if(dst.data[idxY + cX] == 74) break;
    }
    ...
    for( int y = cY, idxY = cY * drawing.cols ; y < drawing.rows ; y++, idxY += drawing.cols )
    {
        for( int x = cX ; x > 0 ; x-- )
        {
            if(dst.data[idxY + x] == 74) break;
